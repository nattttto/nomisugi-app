...
休肝日は**明示的に記録された日だけ**を数える（後述の restDays を参照）。

### 今日の計画（飲み会モード）

飲み始める前に `SessionPlan` を立てられる（`SessionPlanModal`）。

- `mode`（`solo` / `party`）と `endBy`（何時までにするか、任意）を持つ
- 計画の上限は `DrinkingSession.goalAlcoholG` に入る。**プロフィールの目標より、その日の計画が優先される**
- 計画を立てずに1杯目から記録した回は `plan: null`。後から足したフィールドなので、
  それ以前の記録では `undefined` になる

**飲み会の日は「飲むな」では成立しない。** `PacePlanCard` が
「終わりの時刻までに、決めた量で収まるペース」＝ **次の1杯まで何分空ければいいか** を出す
（`calcPacePlan`）。残りが中ジョッキ1杯ぶんに満たないときと、終わりの時刻が未設定のときは出さない。

`projectGoalReachedAt` は「いまのペースのままなら何時に上限へ届くか」を返す。
飲み始めてすぐは分母が小さくて極端な時刻が出るので、**20分経つまでは出さない**。

終わりの時刻の入力（`resolveEndBy`）は、**過ぎている時刻を翌日として解釈する**。
22時に「01:00」と入れたら翌日の1時のこと。

### ワンタップ記録（quickDrinks.ts）

**このアプリの価値は記録が続くかどうかで決まる。** 統計も傾向も介入も、記録が途切れた瞬間に同時に死ぬ。
「記録ボタン → 種類 → サイズ → 記録」の3タップは、酔っている状態では重い。

ホームに「もう1杯」（直前と同じもの）と「よく飲む3種」を出して1タップで入るようにしている。

- 回数は `UserProfile.drinkCounts` に `"種類:サイズ": 回数` の**生カウント**で貯める。
  プロフィールは元々読んでいるので、**追加の読み取りなし**で上位3件を出せる
- カウントの加算（`bumpDrinkCount`）は**記録本体と切り離し、失敗しても無視する**。
  これは便宜であって、失われても記録の正しさには影響しない
- 量と度数を手入力する「その他」は「種類:サイズ」に戻せないので、回数を数えない（`countKey: null`）。
  ただし「もう1杯」は記録そのものから復元できるので使える
- 同数のときはキー順にして並びを安定させる。押す位置が毎回変われば押し間違える
- `drinkCounts` は開いたときにしか読まないので、今日ぶんの増加は次に開いてから反映される

### 休肝日（restDays）

**記録が無い日は「飲まなかった日」ではなく「アプリを開かなかった日」かもしれない。**
以前は飲酒日どうしの間隔から休肝日を推定していたが、記録を忘れただけの日も休肝日に数えてしまっていた。

`users/{uid}/restDays/{飲酒日キー}` に明示的に残す。**ドキュメントIDが日付そのものなので、
同じ日を二重に登録できない。** 範囲検索は `documentId()` で引く。

- 称号（`longestRestStreak`）は**明示的に記録された日だけ**を数える。押して残した事実にだけ称号を付ける
- カレンダーと月間統計では、**実際に飲んだ日が優先**。両方記録されていても飲酒日として出す

### パーソナライズされた介入（personalization.ts）

一般論（「飲み過ぎに注意」）は誰の行動も変えない。自分のデータで示されたときだけ立ち止まる理由になる。

| id | 出すタイミング | 条件 |
|---|---|---|
| `personal-weekday` | 1杯目まで | その曜日の平均が全体平均の1.15倍超 |
| `personal-fifth` | 4杯目 | 5杯目に届いた回の半分以上で目標超過 |
| `personal-water` | 水なしで3杯目 | 水をはさんだ回のほうが0.5杯以上少ない |
| `personal-over-average` | 2杯目以降 | ふだんの平均の1.2倍超 |

**少ない記録で断定すると嘘になる。** 全体は5回、曜日は3回、5杯目は3回、
水は水あり／水なしそれぞれ3回を下回ったらその項目は出さない。
基準値は**終了した回だけ**から作る（進行中の回は合計が途中なので混ぜない）。

### 通知（useNotifications.ts）

ブラウザ通知。**画面を見ていないときにだけ出す**（開いている画面には同じ警告が既に出ている）。
`info` は通知しない。同じ警告で鳴り続けないよう通知済みIDを覚え、その警告が消えたら忘れる。

許可を求めるのは**設定を「入」にした瞬間だけ**。画面を開いただけで求めると、たいてい拒否されて
二度と聞けなくなる。`UserSettings.notificationsEnabled` は後から足した項目なので、
持っていない既存ユーザーは「切」として扱う。

**サーバーから送るプッシュではないので、アプリを閉じている間は届かない。**
閉じていても届く通知は FCM の登録が要る。
...
- **Phase 2（実装済み）**：月間/年間統計・グラフ・カレンダー・飲酒傾向分析・個人平均ペース・
  称号・PWA アイコン
- **Phase 3（一部）**：「今日飲み会？」モード・パーソナライズされた介入・通知
- **記録が続くための土台（実装済み）**：ワンタップ記録・休肝日の明示的な記録・
  Service Worker（ホーム画面に追加できるように）
- **未着手**：オフライン記録（`addDrinkRecord` の `runTransaction` を `increment()` に
  置き換える必要がある。トランザクションはオフラインでは動かない）・
  記録の編集・見逃したサマリの表示・エクスポート・
  AI分析（Claude API キーが要る）・ウェアラブル連携（Fitbit / Google Fit の
  アプリ登録が要る）・閉じていても届くプッシュ通知（FCM の登録が要る）
...
**ホーム画面に追加できるようにするには manifest とアイコンだけでは足りない。**
fetch を扱う Service Worker（`public/sw.js`）が要る。`PwaRegister` が登録するが、
**開発中は登録しない**（古いバンドルをキャッシュから掴んで、直したのに反映されない状態になる）。
